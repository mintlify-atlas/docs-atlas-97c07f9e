...
**Availability Engine** — the logic that intersects the booking type's work calendar with the individual work calendars of every resource in a combination. It respects calendar leaves, honors public holidays (when `hr_holidays` is installed), and excludes time already occupied by other confirmed calendar events. This engine powers both the slot picker in the portal and the conflict-detection constraint that fires on every save.
...
**Optional modules** (enable extra features when installed)

- `hr` — links `resource.resource` records to employees
- `hr_holidays` — respects employee time-off leaves in availability calculation
- `mrp` — enables booking of manufacturing resources
