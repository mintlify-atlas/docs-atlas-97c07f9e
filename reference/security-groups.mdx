...
| Rule name | Model | Filter |
|-----------|-------|--------|
| Resource booking type multi company rule | `resource.booking.type` | `company_id in user.company_ids + [False]` |
| Resource booking multi company rule | `resource.booking` | `type_id.company_id in user.company_ids + [False]` |

A booking type with no company (`company_id = False`) is visible to users of **all** companies.

#### Per-group record rules on `resource.booking`

| Rule name | Applies to | Domain |
|-----------|-----------|--------|
| Resource booking portal rule | `base.group_portal` | `partner_ids child_of user.partner_id` **OR** `message_partner_ids child_of user.partner_id` |
| Resource booking user rule | `resource_booking.group_user` | attendee **OR** follower **OR** `combination_id.resource_ids.user_id = user` |
