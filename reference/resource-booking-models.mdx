...
**Constraint `_check_bookings_scheduling()`** — Decorated with `@api.constrains("resource_booking_ids", "start", "stop")`. Calls `resource.booking._check_scheduling()` whenever the event's `resource_booking_ids`, `start`, or `stop` changes, keeping schedules valid from both sides.
...
**New method `is_available(start_dt, end_dt, domain=None, tz=None)`** — Convenience helper that returns `True` if the resource has a contiguous work interval covering the entire `[start_dt, end_dt]` window. Internally calls `calendar._work_intervals_batch()` with `analyzing_booking=-1` context to count existing bookings as busy time.
