...
<ParamField body="date_end" type="Date">
...
<ParamField body="ics_file" type="Binary">
  The `.ics` file to import. Must be a valid iCalendar file (RFC 5545).
</ParamField>

<ParamField body="date_start" type="Date">
  Import only events whose start date is on or after this value. Leave empty to import all events.
</ParamField>

<ParamField body="date_end" type="Date">
  Import only events whose start date is on or before this value. Leave empty to import all events.
</ParamField>

<ParamField body="partner_id" type="Many2one → res.partner">
  Imported events are associated with this partner.
</ParamField>

<ParamField body="remove_old_events" type="Boolean">
  When enabled, any existing calendar events for the selected partner that fall within the specified date range are deleted before the new events are imported. Use with caution in shared calendars.
</ParamField>
