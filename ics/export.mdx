...
<Steps>
  ...
  <Step title="Select a partner">
    The **Partner** field defaults to the currently logged-in user's partner. ERP managers can change this to any partner in the system. Regular users always export for themselves and do not see this field.
  </Step>
  ...
  <Step title="Generate the file">
    Click **Generate ICS File**. The wizard calls `button_export()`, builds the iCalendar payload, and replaces the form with a download widget.
  </Step>
...
The wizard searches `calendar.event` with the following filters applied in order:
...
<ParamField path="partner_id" type="Many2one (res.partner)" default="Current user's partner">
  The partner whose calendar events are exported. Visible only to users in the `base.group_erp_manager` group; all other users always export their own events.
</ParamField>
